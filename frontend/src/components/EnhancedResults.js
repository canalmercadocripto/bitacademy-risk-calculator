...
const EnhancedResults = ({ results, selectedSymbol, selectedExchange, formData, currentPrice }) => {
  ...
  const calculateProfitTargets = () => {
    ...
    const currentRisk = Math.abs((entry - stop) * results.position.size);
  ...
  const organizeResultsByType = () => {
    ...
    const isProfit = results.profit.amount > 0;
    const profitAmount = Math.abs(results.profit.amount);
    const profitPercentage = Math.abs(results.profit.percentage);
  ...
  const generateProfessionalRecommendations = () => {
    ...
    const riskReward = results.analysis.riskRewardRatio;
    const riskLevel = results.analysis.riskLevel;
    const riskPercentage = parseFloat(formData.riskPercent || 2);
    const accountSize = parseFloat(formData.accountSize);
    const positionSize = results.position.value;
    const winRateAnalysis = calculateWinRateNeeded(riskReward);
    
    // Validar risco
    const maxRiskAmount = (accountSize * riskPercentage) / 100;
    const currentRisk = Math.abs(results.risk.amount);
...
💼 POSIÇÃO:
🪙 Quantidade: ${results.position.size.toFixed(6)} moedas
💵 Valor Total: $${results.position.value.toFixed(2)}
📈 Direção: ${results.position.direction}
...
🛡️ Risco Máximo: $${results.risk.amount.toFixed(2)}

⚖️ ANÁLISE RISK/REWARD:
🎯 Ratio: ${results.analysis.riskRewardRatio.toFixed(2)}/1
📋 Classificação: ${results.analysis.riskLevel}
...
💡 RECOMENDAÇÕES:
${results.analysis.recommendation.join('\n')}
        ...
        <div className="section-card">
          ...
          <div className="position-grid-vertical">
            <div className="position-item">
              ...
              <span className="item-value">{results.position.size.toFixed(6)}</span>
            </div>
            <div className="position-item">
              <span className="item-label">Valor Total:</span>
              <span className="item-value">${results.position.value.toFixed(2)}</span>
            </div>
            <div className="position-item">
              <span className="item-label">Direção:</span>
              <span className={`item-value direction ${results.position.direction.toLowerCase()}`}>
          ...
          <div className="financial-highlight-new">
            <div className={`result-main ${profitLoss.status}`} 
              ...
              <div className="result-position-info">
                Posição: ${results.position.value.toFixed(2)} • Direção: {results.position.direction}
              </div>
            </div>
          </div>
            ...
            <div className="rr-section">
              ...
              <div className="rr-grid">
                <div className="rr-card risk">
                  ...
                  <div className="rr-card-value">-${results.risk.amount.toFixed(2)}</div>
                  <div className="rr-card-desc">{results.risk.percentage.toFixed(1)}% da conta</div>
                </div>
            ...
            <div className="rr-section">
              ...
              <div className="rr-analysis-card">
                <div className="rr-ratio-big">
                  <span className="ratio-number">{results.analysis.riskRewardRatio.toFixed(1)}</span>
                ...
                <div className="rr-classification">
                  ...
                  <span className="classification-value" style={{ color: getRiskLevelColor(results.analysis.riskLevel) }}>
                    {results.analysis.riskLevel}
                  </span>
                </div>
                <div className="rr-explanation">
                  Para cada $1 arriscado, o potencial é de ${results.analysis.riskRewardRatio.toFixed(1)} de lucro
                </div>
                <div className="win-rate-needed" style={{ marginTop: '10px', padding: '12px', background: 'rgba(255,255,255,0.1)', borderRadius: '6px', border: `1px solid ${calculateWinRateNeeded(results.analysis.riskRewardRatio).classification.color}` }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', fontSize: '0.85em' }}>
                    <div>
                      ...
                      <strong>{calculateWinRateNeeded(results.analysis.riskRewardRatio).breakeven.toFixed(1)}%</strong>
                    </div>
                    <div>
                      <span style={{ color: '#6c757d' }}>Lucro:</span><br/>
                      <strong>{calculateWinRateNeeded(results.analysis.riskRewardRatio).profitable.toFixed(1)}%</strong>
                    </div>
                    <div>
                      <span style={{ color: '#6c757d' }}>Conservador:</span><br/>
                      <strong>{calculateWinRateNeeded(results.analysis.riskRewardRatio).conservative.toFixed(1)}%</strong>
                    </div>
                  </div>
                  <div style={{ marginTop: '8px', fontSize: '0.8em', fontWeight: '600', color: calculateWinRateNeeded(results.analysis.riskRewardRatio).classification.color }}>
                    {calculateWinRateNeeded(results.analysis.riskRewardRatio).classification.level} - {calculateWinRateNeeded(results.analysis.riskRewardRatio).classification.desc}
                  </div>
                </div>
              </div>
            </div>
